 ...
 *
 * 整合テストは実 Postgres に接続するが、DATABASE_URL は .env から採用しない (production
 * :55432 をローカル vitest が黙って拾う事故の再発防止)。DB を渡すのは CI の env 注入か
 * ACTRADECK_TEST_DATABASE_URL (明示 opt-in)。production port を指す接続文字列は fail-loud で
 * 拒否する。ロジックの単一出所は @actradeck/event-model の test-db-guard.ts
 * (backend/webui/sidecar と共有・旧 3 コピーの重複を解消)。
 */
