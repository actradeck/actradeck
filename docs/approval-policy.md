...
| `db-drop`          | `DROP TABLE` / `TRUNCATE TABLE` / `DROP DATABASE` / `dropdb` / `DROP SCHEMA` / `DROP OWNED BY` / `mysqladmin … drop` / `dropDatabase(` / `drop_database(` / `FLUSHALL` / `FLUSHDB` (literal list; not exhaustive — see note below) | **ON**  |
