...
[公開取込コントラクト](../../ingestion-contract.md) の `NormalizedEvent` へ写像し、
...
- **この adapter は client 側 redaction を持ちません** (依存ゼロのため)。secret に対する唯一の
  防御は ActraDeck **backend の ingress redaction 床** ([契約 §5](../../ingestion-contract.md#5-ingress-redaction-床-保存前-redaction)・
  保存前に無条件適用) です。
