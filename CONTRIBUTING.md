...
## Mirror model & how your PR lands

This public repository is a **one-way curated mirror** of a private canonical repo.
Its git history is **synthetic** — a single snapshot commit per sync — because the
tree is generated by a pipeline that strips internal coupling (private URLs, developer
paths, AI co-author trailers) and runs a secret/leak gate before anything is published.
That keeps the public tree clean, but it has one consequence worth being upfront about:
**the mirror is regenerated on each sync**, so a PR merged only on the mirror would be
overwritten. Here is how we make sure that never costs you your contribution.

**Your PR does not get lost.** When your PR is accepted, a maintainer imports it into
the private canonical with `scripts/import-oss-pr.sh`, which applies your patch via
`git am`. This **preserves your original authorship** (author and committer identity)
...
*does* serve as your durable public credit is:

1. **the PR itself**, closed as imported (it stays in the repo's PR history), and
2. **[`CONTRIBUTORS.md`](./CONTRIBUTORS.md)**, a permanent record that ships with the
   mirror — the import tool adds a line crediting you and linking back to your PR.

**Conflicts can happen.** Because the mirror is produced by rewriting private URLs/handles
and excluding some internal files, a patch generated against the mirror occasionally does
not apply cleanly onto the private canonical. When that happens the maintainer resolves it
by hand during import — no action is needed from you unless we ask a question on the PR.

None of this changes what you do: open a focused PR following the
