...
# ============================================================================
# INV-THIRDPARTY-LICENSES-CURRENT  (drift tripwire for the attribution inventory)
# ============================================================================
# THIRDPARTY_LICENSES.md is generated by scripts/gen-thirdparty-licenses.sh from the
# installed workspace (`pnpm licenses list --json`). The committed file is the linux-x64
# view (platform-constrained native packages appear for the install platform only), so
# the exact-match check runs only in that regime — elsewhere it is reported as SKIP,
# never as a pass. GREEN: the committed file equals a fresh regeneration. RED: a copy
# with one row dropped must fail `--check` (falsifiable). NO-RAW: the generated table
# must pass the same absolute-path / secret-shape scan as the SBOM.
GEN_TP="$ROOT/scripts/gen-thirdparty-licenses.sh"
if [ ! -x "$GEN_TP" ]; then
  ng "INV-THIRDPARTY-LICENSES-CURRENT: scripts/gen-thirdparty-licenses.sh missing or not executable"
elif [ "$(uname -s)" != "Linux" ] || [ "$(uname -m)" != "x86_64" ]; then
  echo "SKIP  INV-THIRDPARTY-LICENSES-CURRENT: exact-match check is linux-x64 only (CI regime; native deps differ per platform)"
elif ! bash "$GEN_TP" --stdout > "$WORK/thirdparty.gen.md" 2>"$WORK/thirdparty.err"; then
  ng "INV-THIRDPARTY-LICENSES-CURRENT: generator failed (need installed deps: run 'pnpm install'): $(head -1 "$WORK/thirdparty.err")"
else
  if bash "$GEN_TP" --check >/dev/null 2>&1; then
    ok "INV-THIRDPARTY-LICENSES-CURRENT: THIRDPARTY_LICENSES.md matches a fresh regeneration"
  else
    ng "INV-THIRDPARTY-LICENSES-CURRENT: THIRDPARTY_LICENSES.md is stale — run scripts/gen-thirdparty-licenses.sh and commit it"
  fi
  # RED: drop the last package row from a copy -> --check must fail on that copy.
  sed '$d' "$ROOT/THIRDPARTY_LICENSES.md" > "$WORK/thirdparty.mut.md"
  if THIRDPARTY_LICENSES_FILE="$WORK/thirdparty.mut.md" bash "$GEN_TP" --check >/dev/null 2>&1; then
    ng "INV-THIRDPARTY-LICENSES-CURRENT: DEAD GATE — a copy missing a package row still passed --check"
  else
    ok "INV-THIRDPARTY-LICENSES-CURRENT: --check FAILS when a package row is dropped (falsifiable)"
  fi
  if sbom_scan_noraw "$WORK/thirdparty.gen.md" >/dev/null 2>&1; then
    ok "INV-THIRDPARTY-LICENSES-CURRENT: generated inventory carries no absolute path / secret shape (NO-RAW)"
  else
    ng "INV-THIRDPARTY-LICENSES-CURRENT: generated inventory tripped the NO-RAW scan"
  fi
fi
