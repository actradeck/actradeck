...
export default defineConfig({
  test: {
    ...
    coverage: {
      provider: "v8",
      include: ["src/**/*.ts"],
      exclude: ["src/index.ts", "src/lib/deps.ts", "src/lib/fake.ts", "src/**/*.{test,spec}.ts"],
      ...
      thresholds: {
        ...
        // injected Deps): 99.2 / 95.16 / 100 / 99.55. Floors sit 4-5pt below so a real regression
        // (a whole command's error branch going uncovered) trips, but v8/node cross-env branch-
        // counting jitter does not.
        statements: 95,
        branches: 90,
        functions: 95,
        lines: 95,
