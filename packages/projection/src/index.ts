...
// SUMMARY_SUBJECT_CAP / boundTurnSummary は leaf module bound.ts へ移設 (A1 TDA-4: work-items.ts が
// 本 module から import すると index ↔ work-items の 2 ノード循環になるため共有点を leaf 化)。
// 公開 API は本 re-export で不変 (backend replay-store は従来どおり "@actradeck/projection" から引く)。
export { SUMMARY_SUBJECT_CAP, boundTurnSummary } from "./bound.js";
 ...
 *  - turn.started → payload.prompt_summary / turn.completed → payload.response_summary (ADR 019f47c2)
 *  - session.ended → payload.reason / turn.failed → payload.error ?? payload.reason
 *  (diff.updated / heartbeat / turn.plan.updated 等 構造的 subject 無し → undefined)
 */
export function deriveActionSubject(eventType: string, payload: unknown): string | undefined {
  ...
  switch (eventType) {
    ...
    // turn.started / turn.completed は「依頼(prompt要約)」「応答(response要約)」を subject に載せる
    //   (ADR 019f47c2・CC パリティ + gemini 観測の KPI「何をしているか」回復)。出所は payload の
    //   有界要約フィールド (adapter/normalizer が summarize で作る)。**redacted payload allowlist**
    //   由来ゆえ INV-CURRENT-ACTION-NO-LEAK / INV-REPLAY-SUBJECT-NO-LEAK を維持する (raw prompt 全文
    //   は載せない・summary は backend ingress 床で redaction 済)。欠落 (旧行/tool 起因の turn) は undefined。
    case "turn.started":
      // SEC-1: adapter は secret を分割しない sanity-cap のみで送るため、有界化は床の後 (redacted 値) の
      //   ここで行う (INV-REDACTION-SUMMARY-STRADDLE (D))。
      return boundTurnSummary(payloadString(p, "prompt_summary"));
    case "turn.completed":
      return boundTurnSummary(payloadString(p, "response_summary"));
    case "session.ended":
