 ...
 * 除去量は導出を書かないと意味を持たないので、導出と測定 tree を必ず併記する。
 * **導出**: `git ls-files` を `SCAN_SOURCE_EXTENSIONS` で絞り、各ファイルを
 * `ts.createSourceFile` して `getLeadingCommentRanges` / `getTrailingCommentRanges` が
 * 列挙した範囲のうち `SingleLineCommentTrivia` で、**その行に非空白が先行する**もの (= 末尾
 * コメント) を数える。除去文字数は各コメントの `end - pos` に**直前の空白 / タブの連**を足した和。
 * marker oracle の probe 点数は走査集合の**非空白行**の総数 (空行には marker を注入しないため)。
 * **実測 (commit 939200a の tree)**: 走査集合 **620 file** / 末尾行コメント **2,262 本** を
 * **306 file** が持ち、除去文字数 **58,864 字**、marker oracle の probe **148,159 点**。
 * いずれも走査集合が変われば動く値で、**測定した tree を離れては意味を持たない** —
 * CHANGELOG は数値を複写せず本節を参照する。
 * **過去値との差の開示**: 以前 CHANGELOG が載せていた「commit 7014618 で 56,166 字」は、
 * 同 tree を上の導出で測ると **58,414 字**になる (コメント 1 本あたり 1 字少なく数えていた)。
 * コメント本数 2,248 / 306 file は同 tree で逐語に再現するので、ずれていたのは字数の規則だけ。
 *
