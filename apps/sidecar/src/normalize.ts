 ...
 *   **残る非被覆は「除外をクラスの外で表現した綴り」** (SEC-LSI-R2-3): ① 量化 shorthand `\S*` `\s+`
 *   (と `s` flag つきの `.`) は census にも結合検査にも掛からない ② **負先読み形**
 *   `(?:(?!\|)(?!;)(?!&)(?!\n)[\s\S]{1}){0,512}` はクラスが量化済みゆえ census を位置一致で通り、
 *   `[\s\S]` は何も除外しないので結合検査も構造ゲートも通る — それでいて**挙動は出荷形
 *   `[^|;&\n]{0,512}` と同一**である (3 ゲートすべて素通り・実証済み)。lazy `*?` と `{n,m}` は抽出される。seed 軸は綴りに依らず全スキャン regex を
 ...
 * **残るのは「除外をクラスの外で表現した綴り」** — 量化 shorthand (`\S*` `\s+` / `s` flag つきの `.`) と
 * **負先読み形** (`(?:(?!\|)(?!;)(?!&)(?!\n)[\s\S]{1}){0,512}`・挙動は出荷形と同一なのに 3 ゲートすべてを
 * 通る・SEC-LSI-R2-3 実証) — で、その綴りには機械的強制が届かない = **規律を人が守る必要がある**。
 * 構造ゲートの分離子判定は綴り軸 (`startsWith("[^")`) と**受理集合**軸 (`spansArbitraryText`) の**論理和**で
 * 行い、判定は `isSeparatorGapClass` の単一出所 (走査行と軸 fixture が同じ関数を呼ぶ)。和が単独で救うのは
 * 「結合検査が exemption で迂回された規則」に限り、負先読み形は和でも止まらない。判定を行う (TDA-LSI-1: 受理集合軸への置換は「英数字を 1 つも受理しない否定クラス」`[^a-zA-Z0-9|]` を落とした)。
