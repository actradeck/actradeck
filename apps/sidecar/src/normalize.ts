 ...
 *   第 5 軸でも閉じない)、② 結合検査の文字 universe が**依然有限** — task 01a0574f-521a で非 ASCII 分離子を
 *   手写し 5 文字から `/[\p{Zs}\p{Zl}\p{Zp}]/u` の BMP 走査 (18 code point) へ導出化して穴を縮めたが、
 *   Unicode 分離子でもなく universe にも無い文字だけを除外する gap クラスは依然結合検査を素通りし
 ...
 *   **位置集合**」を要求し、群括り `(?:[^|;&\n])*` / capture `([^|;&\n])*` / クラス alternation
 *   `(?:[^|;&\n]|x)*` / 未量化 `[abc]` / **群括りの CR 幅版** (CR 幅は群括り 1 形でのみ計測) /
 *   **末尾に inert な phantom `(?:\[\s\S]*)?` を足して
 *   本数だけ合わせた形** (phantom は 2 形) を **床 (fail-closed) で RED** にする — **実測で塞げたのは
 *   計 8 形・うち phantom 3 形** (SEC-LSI-2: 本数一致版は phantom 形を素通りした)。現行 17 は位置一致で緑・escape 済み
 *   `\[abc\]` も緑 = false RED なし。**塞げたのはここに列挙した実測形**であって、census を回避する
 *   綴りが他に無いことの証明ではない)。
 *   **残る非被覆の特徴づけ (SEC-LSI-R3-2 で列挙をやめた)**: **gap の受理集合をクラス以外の構文で
 *   表現した綴り**には 3 ゲートのいずれも届かない。実測済みの例は量化 shorthand (`\S*` `\s+` /
 *   `s` flag つきの `.`)・**負先読み形** `(?:(?!\|)(?!;)(?!&)(?!\n)[\s\S]{1}){0,512}` (クラスが
 *   量化済みゆえ census を位置一致で通り、`[\s\S]` は何も除外しないので結合検査も構造ゲートも通る)・
 *   **class-free alternation gap** `(?:\w|\s|…){0,512}` の 3 つで、いずれも**挙動は出荷形
 *   `[^|;&\n]{0,512}` と同一**である (3 ゲートすべて素通り・実証済み)。**これは例示であって網羅ではない** —
 *   R2 で「2 系統」と列挙した記述は R3 の class-free alternation で反証された。lazy `*?` と `{n,m}` は
 *   抽出される。seed 軸は綴りに依らず全スキャン regex を測るので、これらの綴りで失われるのは 2 ゲートの
 *   適用であって計測ではない。現行 17 に該当する綴りは無い。
 *   **走査行そのものは無観測** (TDA-LSI-R3-1 / SEC-S4 / QA-N1・N4・base 同値): 判定の単一出所化
 *   (`isSeparatorGapClass` / `censusVerdict`) が守るのは verdict の**中身**であって、走査行が実際に
 *   その verdict を照合しているかではない。空 verdict 差し替え / 走査の恒真化 / 構造ゲートループ先頭への
 *   早期 return 挿入はいずれも無音で通る (3 レーン独立実測)。是正は走査範囲の変更ゆえ
 *   task 01a058f0-b045 (v0.9・full 監査) で行う。
 ...
 * capture `([^|;&\n])*` / クラス alternation / 未量化 `[abc]` / phantom で本数を合わせた形 3 種・計 8 形) は
 * class census が着地自体を RED にするので、**実測したこれらの綴りで**ゲートを避ける経路は塞がっている
 * (網羅の主張はしない)。**残るのは「gap の受理集合をクラス以外の構文で表現した綴り」** — 量化 shorthand /
 * 負先読み形 / class-free alternation gap が実測例で、いずれも挙動は出荷形と同一なのに 3 ゲートすべてを
 * 通る (SEC-LSI-R2-3 → R3-2)。列挙ではなくこの特徴で読むこと。その綴りには機械的強制が届かない =
 * **規律を人が守る必要がある**。
 * 構造ゲートの分離子判定は綴り軸 (`startsWith("[^")`) と**受理集合**軸 (`spansArbitraryText`) の**論理和**で
 * 行い、判定は `isSeparatorGapClass` の単一出所 (走査行と軸 fixture が同じ関数を呼ぶ)。受理集合軸への
 * **置換**は「英数字を 1 つも受理しない否定クラス」(`[^a-zA-Z0-9|]`) を落としたので和にしてある
 * (TDA-LSI-1)。和が単独で救うのは「結合検査が exemption で迂回された規則」に限り、上の残余綴りは和でも
 * 止まらない。受理集合軸により正のクラスで綴られた広い gap (`[\w\s-]*`) も分離子として扱う。
 */
