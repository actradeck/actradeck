 ...
 *
 * 背景: db-drop の literal は「追加のみ・削除禁止」の規律で育っている。行を足したのに
 * ベクタを足さない (逆に、ベクタだけ足して行を足さない) 編集は、既存の仕掛け (label 二方向
 * lock・LINEAR の exact count pin・公開ベンチの divergence 件数) にも落ちる (base 対照で実測
 * RED・QA-DE-3) が、失敗が遠い場所で名前なく出る。本 test が足すのは**失敗を db-drop 局所で
 * 名前付きに落とす**ことと**逆向きの帰属** (どの行がどのベクタを担うか) である。
